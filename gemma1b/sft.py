...
class TrainingManager:
    ...
    def setup_model_and_tokenizer(self):
        ...
        # Load config
        print("Loading Gemma3 config...")
        config = AutoConfig.from_pretrained("google/gemma-3-1b-it", trust_remote_code=True)
        
        # Remove vision_config if present
        ...
        # Load model
        print("Loading Gemma3ForCausalLM model...")
        ...
        # Optionally set a very large model_max_length (not strictly required):
        # self.tokenizer.model_max_length = 100000  # or some large number
    ...
    def load_and_process_datasets(self, train_file, test_file, data_fraction):
        ...
        def preprocess_function(batch):
            ...
            # IMPORTANT: No truncation or max_length
            # We'll let the model handle large contexts (watch out for OOM).
            tokenized = self.tokenizer(
                out_texts,
                truncation=False,  # <-- No truncation
                padding=False      # <-- No padding to max_length either
            )
            return tokenized
...
def parse_args():
    ...
    # We keep --max_seq_length in case you want to experiment,
    # but it's no longer used in the tokenization step.
...
def main():
    ...
    # Load and process datasets (with debug prints)
